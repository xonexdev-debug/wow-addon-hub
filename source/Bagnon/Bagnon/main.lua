...
function Bagnon:GetFrame(frameID)
	if self.framesByID and self.framesByID[frameID] then
		return self.framesByID[frameID]
	end

	for i, frame in pairs(self.frames) do
		if frame:GetFrameID() == frameID then
			if self.framesByID then
				self.framesByID[frameID] = frame
			end
			return frame
		end
	end
...
function Bagnon:CreateFrame(frameID)
	local frame = self.Frame:New(frameID)
