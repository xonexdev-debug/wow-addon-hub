...
Genie.CONSTANT = {
    ...
    COMM_SYNC = 'GENIE_SYNC',
    COMM_DATA = 'GENIE_DATA',
    COMM_TIME = 'GENIE_TIME',
}
Genie.CMD = {
    name = L['Genie'],
    handler = Genie,
    ...
    args = {
		...
		bag_select = {
			...
			values = function () 
				local b = {}
					--bags
					for i, slot in pairs(Genie.CONSTANT.BAGS.BAGSLOTS) do 
						local bag = Genie.CLASS.Bag:New(slot, Genie.CONSTANT.BAGS)
						if bag:GetName() == L['Unknown'] then
							b[i.. Genie.CONSTANT.BAGS.SHORT ..slot] = slot 
						elseif slot > 0 then
							b[i.. Genie.CONSTANT.BAGS.SHORT ..slot] = slot.. ' ('.. bag:GetName() ..')' 
						else
							b[i.. Genie.CONSTANT.BAGS.SHORT ..slot] = bag:GetName()
						end
					end				
					return b
				end
...
function Genie:OnInitialize()
	...
	-- guildbank
	ac:RegisterOptionsTable(L["Genie"]..": "..Genie.GUI.Guildbank.name, Genie.GUI.Guildbank)
	self.optionsFrame.Guildbank = LibStub("AceConfigDialog-3.0"):AddToBlizOptions(L["Genie"]..": "..Genie.GUI.Guildbank.name, Genie.GUI.Guildbank.name, "Genie")
...
function Genie:OnEnable()
    ...
    if IsInGuild() then 
        GuildRoster() 
	    self:RegisterComm(Genie.CONSTANT.COMM_SYNC)
    	self:RegisterComm(Genie.CONSTANT.COMM_DATA)
    	self:RegisterComm(Genie.CONSTANT.COMM_TIME)
...
function Genie:Sort(container)
	...
	local function ItemComesBefore(a, b)
		if a.item < b.item then
			return true
		elseif b.item < a.item then
			return false
		end
	...
	local function TargetSort()
		...
		for i, item in ipairs(items) do
			sorted[i] = { item = item, index = i }
		end

		table.sort(sorted, ItemComesBefore)

		for i, entry in ipairs(sorted) do
			local item = entry.item
			local target = targets[i]
			if target and (item:GetTarget()[1] ~= target[1] or item:GetTarget()[2] ~= target[2]) then
				item:SetTarget(target[1], target[2])
				swapsNeeded = swapsNeeded + 1
			end
		end
	end
	...
	--[[
		...
		selectionsort is kept as a fallback, but TargetSort avoids the O(n^2) scan
	--]]
	if container == nil then container = Genie.theWorks.container end
	...
	for family, bags in pairs(container.familyToBag) do
		for i, item in pairs(container:GetItems(bags)) do
			if item:GetIgnore() == false then
				...
				items[#items + 1] = item
				targets[#targets + 1] = item:GetTarget()
			end
		end
	--[[
		Sorting
	...
	--]]
		if Genie.db.profile.sortalgo == 'sort_quick3' then
			TargetSort()
		elseif Genie.db.profile.sortalgo == 'sort_insert' then
			InsertionSort()
...
function Genie:DoFastSwap(container)
	...
	local function IsSlotLocked(bag, slot)
		local cBag = container:GetBag(bag)
		if cBag then
			return select(3, cBag:GetContainerItemInfo(slot))
		end
		return false
	end
	...
	for bag, contents in pairs(container.SwapGrid) do
		for slot, target in pairs(contents) do
            ...
            elseif bag ~= container.SwapGrid[bag][slot][1] or slot ~= container.SwapGrid[bag][slot][2] then
				...
				local locked1 = IsSlotLocked(bag, slot)
				local locked2 = IsSlotLocked(tBag, tSlot)
				if locked1 or locked2 then
					...
					elseif IsSlotLocked(bag, slot) or IsSlotLocked(tBag, tSlot) then
						blockedThisRound = true
					end
					changesThisRound = true
					return false
	...
	for bag, contents in pairs(container.SwapGrid) do
		for slot, target in pairs(contents) do
			if IsSlotLocked(bag, slot) or IsSlotLocked(target[1], target[2]) then
				blockedThisRound = true
			end
		end
	end
...
function Genie:GUILD_ROSTER_UPDATE()
	...
	Genie.GUI.Guildbank.args.GB_LockGuildBank.disabled = hide
    if (IsInGuild()) then
        self.currentGuild = GetGuildInfo("player")
    else
        self.currentGuild = nil
...
function Genie:DoBag()
	...
	Genie.theWorks.container = Genie.CLASS.Container:New(L['Bags'])
	Genie.theWorks.container:FillWithGameContainer(Genie.CONSTANT.BAGS)
	self:Do()
end
...
function Genie:FamilyIgnore(info, value)
	...
	if Genie.CLASS.Family:Exists(params[1]) then
        local f = Genie.CLASS.Family:New(params[1])
            Genie:DebugPrint('FamilyIgnore', f:GetName())
            if #params == 1 then
            ...
            elseif #params == 2 then
                local typeName = tostring(params[2]):upper()
                local typeNumber = tonumber(params[2])
                Genie:DebugPrint('FamilyIgnore:#2', typeName, typeNumber)
                for name, v in pairs(Genie.Container) do
                    if name == typeName or v.TYPE == typeNumber then
                        Genie:DebugPrint('SetIgnore', name, v.TYPE)
                        f:SetIgnore(v)
                        break
                    end
                end
...
function Genie:LookForNewGuildProfile()
    if Genie.currentGuild and self.db.factionrealm[Genie.currentGuild].sync then
        self:SendCommMessage(Genie.CONSTANT.COMM_SYNC, 'REQUEST', "GUILD")
    end
end
function Genie:RequestProfileSync()
    if Genie.currentGuild and self.db.profiles[Genie.db.factionrealm[Genie.currentGuild].useProfile] then
        self:SendCommMessage(Genie.CONSTANT.COMM_DATA, Genie.CONSTANT.COMM_SYNC, "WHISPER", Genie.db.factionrealm[Genie.currentGuild].syncWith)  
    end
end
function Genie:OnCommReceived(prefix, message, distribution, sender)
    ...
    if self.db.factionrealm[Genie.currentGuild].sync and sender ~= UnitName("player") then    
        ...
        if (prefix == Genie.CONSTANT.COMM_SYNC) and distribution == "GUILD" then
            --incoming sync request
            if guildProfile and guildProfile.date then
                self:SendCommMessage(Genie.CONSTANT.COMM_TIME, self:Serialize(guildProfile.date), "GUILD")
            end
        end
        if (prefix == Genie.CONSTANT.COMM_TIME) then
            --process
        ...
        if (prefix == Genie.CONSTANT.COMM_DATA) then       
            if message == Genie.CONSTANT.COMM_SYNC then
                --send
                self:SendCommMessage(Genie.CONSTANT.COMM_DATA, self:Serialize(self.db.profiles[Genie.db.factionrealm[Genie.currentGuild].useProfile]), "WHISPER", sender)
...
function Genie.CLASS.Bag:New(bagSlot, type)
	local self = setmetatable({}, { __index = Genie.CLASS.Bag, __tostring = Genie.CLASS.Bag.ToString })
		...
		self.type = type -- types: CONSTANT.BAGS/BANK/GUILDBANK
...
function Genie.CLASS.GuildBag:IsEnabled()
    if Genie.UTILS.GetGuildBankOption('gbt') then
        -- gbt was set, use it
        return Genie.UTILS.GetGuildBankOption('gbt', self.bagSlot)
    else
        return IsGuildLeader(UnitName("player"))
    end
end
...
function Genie.CLASS.Container:FillWithGameContainer(containerType)
	...
	if self.type == Genie.CONSTANT.GUILDBANK and Genie.atGuildBank == true then
		...
		--SetCurrentGuildBankTab(slot)
		return true
	elseif (self.type == Genie.CONSTANT.BANK and Genie.atBank == true) or self.type == Genie.CONSTANT.BAGS then
...
function Genie.CLASS.Item:ToString()
	...
	--return str
	return tostring(self.Name) .. '['.. tostring(self.ItemID) ..']'
	--return '['.. tostring(self.ItemID) ..']'
end
...
function Genie.CLASS.Item:GenerateSortString()
    local cacheKey = self:GetSortCacheKey()
    if Genie.SortStringCache[cacheKey] then self.SortString = Genie.SortStringCache[cacheKey] else
	...
	for ranking, criteria in Genie.UTILS.pairsByKeys(Genie.db.profile.classranking) do
        ...
        Genie.SortStringCache[cacheKey] = sortstring
        self.SortString = Genie.SortStringCache[cacheKey]
    end
end
function Genie.CLASS.Item:GetSortCacheKey()
		local cacheKey = {}
		cacheKey[#cacheKey + 1] = tostring(self.ItemID or 0)

		for ranking, criteria in Genie.UTILS.pairsByKeys(Genie.db.profile.classranking) do
			if criteria and criteria['enabled'] ~= false then
				local criteriaType = criteria['type']
				local criteriaName = criteria['name'] or ''
				local criteriaKey = criteria['criteria'] or ''
				local value = ''

				if criteriaType == 'family' then
					local family = Genie.CLASS.Family:New(criteriaName)
					if family then
						value = tostring(family:Contains(self))
					end
				elseif criteriaType == 'combined' then
					value = tostring(self:Search(criteriaKey))
				else
					value = tostring(self[criteriaKey])
				end

				cacheKey[#cacheKey + 1] = tostring(ranking) .. '=' .. tostring(criteriaType) .. ':' .. tostring(criteriaName) .. ':' .. tostring(criteriaKey) .. ':' .. tostring(criteria['invert']) .. ':' .. tostring(criteria['max']) .. ':' .. value
			end
		end

		return table.concat(cacheKey, '|')
	end
...
function Genie.CLASS.Family:GetIgnore(container)
    if Genie.db.profile.customFamilies[self.name].ignore then
        if not next(Genie.db.profile.customFamilies[self.name].ignore) then
            -- no container specified within ignore, ignore them all
            return true        
        elseif container then
            -- check
            return Genie.db.profile.customFamilies[self.name].ignore[container.TYPE]
        end
    end
    return false
end

function Genie.CLASS.Family:SetIgnore(container, ignore)
    if container then
        if not Genie.db.profile.customFamilies[self.name].ignore then Genie.db.profile.customFamilies[self.name].ignore = {} end
        -- a specific container was defined
        if  Genie.db.profile.customFamilies[self.name].ignore[container.TYPE] then
            Genie.db.profile.customFamilies[self.name].ignore[container.TYPE] = ignore
        else
            Genie.db.profile.customFamilies[self.name].ignore[container.TYPE] = true
        end
    else    
        if Genie.db.profile.customFamilies[self.name].ignore then
            Genie.db.profile.customFamilies[self.name].ignore = ignore
        else
            Genie.db.profile.customFamilies[self.name].ignore = {}
        end
    end
end
