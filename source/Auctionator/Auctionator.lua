...
local function Atr_OptimizeData()

	local historyItems = 0;

	if (AUCTIONATOR_PRICING_HISTORY) then
		for itemName, hist in pairs (AUCTIONATOR_PRICING_HISTORY) do
...
local function Atr_SlashCmdFunction(msg)
	...
	elseif (cmd == "optimize") then
		...
		zc.msg_atr ("memory usage before optimize: "..Atr_GetAuctionatorMemString());
		local historyItems = Atr_OptimizeData();
		zc.msg_atr ("optimized pricing history items: "..historyItems);
		zc.msg_atr ("memory usage after optimize: "..Atr_GetAuctionatorMemString());
	...
	elseif (Atr_HandleDevCommands and Atr_HandleDevCommands (cmd, param1, param2)) then
		...
		zc.msg_atr (ZT("unrecognized command").." - /atr mem, /atr optimize, /atr clear fullscandb, /atr clear posthistory");
	end
