...
function AtrScan:AddScanItem (name, stackSize, buyoutPrice, owner, numAuctions, curpage)
	...
	for i = 1, numAuctions do
		...
		self.scanData[#self.scanData + 1] = sd;
		
		local itemPrice = 0;
		if (stackSize and stackSize > 0 and buyoutPrice) then
			itemPrice = math.floor (buyoutPrice / stackSize);
		end
...
local function Atr_PrepareScanSortKeys (scn)

	scn.sortNameLC = string.lower (scn.itemName or "");
	scn.sortPrice = 0;

	if (scn.absoluteBest and scn.absoluteBest.stackSize and scn.absoluteBest.stackSize > 0) then
		scn.sortPrice = zc.round (scn.absoluteBest.buyoutPrice / scn.absoluteBest.stackSize);
	end
...
local function Atr_SortScans (x, y)
	...
	local xname = x.sortNameLC or string.lower (x.itemName or "");
	local yname = y.sortNameLC or string.lower (y.itemName or "");
	local xprice = x.sortPrice or 0;
	local yprice = y.sortPrice or 0;
	
	if (gSortScansBy == ATR_SORTBY_NAME_ASC) then		return xname < yname;		end
	if (gSortScansBy == ATR_SORTBY_NAME_DES) then		return xname > yname;		end
...
--			ownerCode = sd.owner;
		...
		local stackSize = sd.stackSize or 0;
		local buyoutPrice = sd.buyoutPrice or 0;
		local key = "_"..stackSize.."_"..buyoutPrice.."_"..ownerCode..dataType;
		...
		if (conddata[key]) then
			...
			data.itemPrice		= stackSize > 0 and buyoutPrice / stackSize or 0;
